/*
 ...
 Foundation, Inc., 59 Temple Place - Suite 330, Boston, MA 02111-1307, USA.
    ...
    The main difference between the Broadcom controller and OneNAND is that
    the Broadcom NAND controller has only a 512B cache (bufferram) regardless
    of the flash chip, whereas OneNAND has multiple bufferram's to match the
    page size.  This complicates this driver quite a bit, because, for large
    page flash (2K page) we have to read in all 4 slides before we know for
    sure whether a page is bad.
 ...
 *
 */
static int brcmnand_read_bbt (struct mtd_info *mtd, uint8_t *buf, int page,
    int num, int bits, int offs, int reserved_block_code)
{
    int res, i, j, act = 0;
    ...
    while (totlen)
    {
        ...
        if (res < 0)
        {
            if (retlen != len)
            {
                printk (KERN_INFO "brcmnand_bbt: Error reading bad block "
                    "table\n");
                return res;
            }
            printk (KERN_WARNING "brcmnand_bbt: ECC error while reading bad "
                "block table\n");
        }
        ...
        for (i = 0; i < len; i++)
        {
            ...
            for (j = 0; j < 8; j += bits, act += 2)
            {
                ...
                {
                    printk(KERN_DEBUG "nand_read_bbt: Reserved block at "
                        "0x%08x\n", ((offs << 2) +
                        (act >> 1)) << this->bbt_erase_shift);
...
/**
 * brcmnand_read_abs_bbt - [GENERIC] Read the bad block table starting at
 *  a given page
 * @mtd:    MTD device structure
 * @buf:    temporary buffer
    ...
    struct nand_bbt_descr *td, int chip)
{
        ...
        for (i = 0; i < this->numchips; i++)
        {
            if (chip == -1 || chip == i)
                res = brcmnand_read_bbt (mtd, buf, td->pages[i],
                    this->chipSize >> this->bbt_erase_shift, bits, offs,
    ...
    else
    {
        res = brcmnand_read_bbt (mtd, buf, td->pages[0], 
            (uint32_t) (this->mtdSize >> this->bbt_erase_shift), bits, 0,
            td->reserved_block_code);
        if (res)
        {
 ...
 */
static int brcmnand_scan_read_raw(struct mtd_info *mtd, uint8_t *buf,
 ...
 */
static int brcmnand_scan_write_bbt(struct mtd_info *mtd, loff_t offs,
    size_t len, uint8_t *buf, uint8_t *oob)
{
    struct mtd_oob_ops ops;
    struct brcmnand_chip *this = mtd->priv;
    int ret;
...
/**
 * brcmnand_read_abs_bbts - [GENERIC] Read the bad block table(s) for all
 *  chips starting at a given page
 * @mtd:    MTD device structure
 * @buf:    temporary buffer
 * @td:     descriptor for the bad block table
 ...
 */
static int brcmnand_scan_block_full(struct mtd_info *mtd,
    struct nand_bbt_descr *bd, loff_t offs, uint8_t *buf, size_t readlen,
    int scanlen, int len)
{
    int ret, j;
 ...
 */
static int brcmnand_scan_block_fast(struct mtd_info *mtd,
    struct nand_bbt_descr *bd, loff_t offs, uint8_t *buf, int len)
{
    struct mtd_oob_ops ops;
    int j, ret;
    int dir;
    ...
    {
        // MLC: Read last page (and next to last page).
...
/**
 * brcmnand_create_bbt - [GENERIC] Create a bad block table by scanning the
 *  device
 * @mtd:    MTD device structure
 * @buf:    temporary buffer
    ...
    struct nand_bbt_descr *bd, int chip)
{
    ...
    loff_t from;
    size_t readlen;
    PRINTK("-->brcmnand_create_bbt, bbt_erase_shift=%d, this->page_shift"
    ...
    else
    {
        ...
        {
            printk (KERN_WARNING "brcmnand_create_bbt(): chipnr (%d) > "
                "available chips (%d)\n",
    ...
    if (gdebug > 3)
    {
        PRINTK("Starting for loop: from=%0llx bd->options=%08x, startblock="
            "%d numblocks=%d\n", from, bd->options, mtd64_ll_low(startblock),
            mtd64_ll_low(numblocks));
    }
...
/**
 * brcmnand_search_bbt - [GENERIC] scan the device for a specific bad block
 *  table
 * @mtd:    MTD device structure
 * @buf:    temporary buffer
 * @td:     descriptor for the bad block table
 ...
 * in a block.
 */
static int brcmnand_search_bbt (struct mtd_info *mtd, uint8_t *buf,
    struct nand_bbt_descr *td)
{
    struct brcmnand_chip *this = mtd->priv;
    int i, chips;
    ...
    for (i = 0; i < chips; i++)
    {
        if (td->pages[i] == -1)
            ...
            printk(KERN_DEBUG "Bad block table found at page %d, version "
...
/**
 * brcmnand_search_read_bbts - [GENERIC] scan device for bad block table(s)
 * @mtd:    MTD device structure
 * @buf:    temporary buffer
 * @td:     descriptor for the bad block table
 * @md:     descriptor for the bad block table mirror
 ...
 * TBD: Use skip block mechanism, and skip over real bad blocks, so we would
 *  either start at the 1MB offset from bottom and go down, or start from the
 *  bottom and go up, skipping over bad blocks until we reach the 1MB
 *  partition reserved for BBT.
 *
*/
    ...
    struct nand_bbt_descr *td, struct nand_bbt_descr *md, int chipsel)
{
    ...
    for (; chip < nrchips; chip++)
    {
        ...
        {
            page = td->pages[chip];
            PRINTK("There is already a version of the table, go ahead and "
                "write it\n");
            goto write;
        }
...
write_retry:
        printk("%s: write_retry: startblock=%0llx, dir=%d, td->maxblocks=%d,"
        ...
        for (i = skip; i < td->maxblocks; i++)
        {
            ...
            switch ((this->bbt[blockindex] >> (2 * (block & 0x03))) & 0x03)
            {
            case 0x01:
            case 0x03:
                continue;
         ...
         * as we may need to break the BBT into 2 or more tables that a
         * uint32_t can index.
         */
        for (i64 = 0ULL; i64 < numblocks; )
        {
            ...
            {
                printk(KERN_ERR "FIXME: %s: integer index to BBT overflow "
                    "%0llx\n", __FUNCTION__, i64 >> 2);
            }
        ...
        if (res < 0)
        {
            printk (KERN_ERR "brcmnand_bbt: Error during block erase at "
                "%0llx: %d\n", to, res);
            skip++;
            goto write_retry;
        }
        ...
        if (res < 0)
        {
            // THT: If writing reports a bad block, we will skip it, and
            // retry.  Eventually may run out of td->maxblocks
            printk(KERN_INFO "write_bbt returns flash status error at %0llx,"
                " skipping and retrying...\n", to);
            skip++;
            goto write_retry;
        }
...
*/
static inline int brcmnand_memory_bbt (struct mtd_info *mtd,
...
*/
static int brcmnand_check_create (struct mtd_info *mtd, uint8_t *buf,
    struct nand_bbt_descr *bd)
{
...
/**
 * brcmnand_scan_bbt - [NAND Interface] scan, find, read and maybe create
 *  bad block table(s)
 * @mtd:    MTD device structure
 * @bd:     descriptor for the good/bad block search pattern
 *
...
int brcmnand_scan_bbt (struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
    ...
    // We don't read/write the BBT frequently, so let it default to not use
    // DMA to avoid OOM
    this->bbt = (uint8_t*) vmalloc (len);
    if (!this->bbt)
    {
        printk (KERN_ERR "brcmnand_scan_bbt: Out of memory, bbt_erase_shift"
    ...
    if (!td)
    {
        ...
        {
            printk (KERN_ERR "brcmnand_bbt: Can't scan flash and build "
                "the RAM-based BBT\n");
            vfree(this->bbt);
            this->bbt = NULL;
        }
    ...
    if (!buf)
    {
        printk (KERN_ERR "%s: Out of memory 2, bbt_erase_shift=%d, "
 ...
 * Currently, the only accepted command is CLEARBBT, which in itself is a
 * dangerous activity.  The user is assumed to know what he is doing.
 */
static void brcmnand_preprocessKernelArg(struct mtd_info *mtd)
{
    ...
    case NANDCMD_CLEARBBT:
        // Force rescan of BBT (DANGEROUS, may lose Mfg's BIs).
    ...
    for (bOffset=bOffsetStart; bOffset<=bOffsetEnd; bOffset+=mtd->erasesize)
    {
        ...
        /* Skip erasing bad blocks.  If you accidentally/intentionally mark a
         * block as bad, and want to clear it, use BBS to clear it.
         * The exception are the blocks in the BBT area, which are reserved.
         * Here during pre-processing, thre is no BBT, so we cannot assume
         * its existence.
         */
        ...
        if (needBBT)
        {
            printk(KERN_WARNING "%s: Erase failure, marking bad block "
...
static void brcmnand_postprocessKernelArg(struct mtd_info *mtd)
{
    ...
    case NANDCMD_CLEARBBT:
        // already done during pre-processing
        brcmnand_displayBBT(mtd);
        return;
        ...
        bOffset = bOffset + mtd->erasesize)
    {
         ...
         */
#if CONFIG_MTD_BRCMNAND_VERSION < CONFIG_MTD_BRCMNAND_VERS_1_0
        if (gClearBBT == NANDCMD_ERASE && pAddr  >= 0x1fc00000 &&
            pAddr < 0x1ff00000)
            continue;
#else
         ...
         * and want to clear it, use BBS to clear it, as Linux does not
         * offer a way to do it.  The exception are the blocks in the BBT
         * area, which are reserved
         */
        else
        {
            unsigned char oobbuf[64];
            ...
            /* THT: This an be a 36bit integer, NAND controller address space
             * is 48bit wide, minus page size of 2*12, therefore 36bit max
             */
            ...
            for (i=0; i<numpages; i++, page += i*dir)
            {
                ...
                if (!res)
                {
                    if (check_short_pattern (oobbuf, this->badblock_pattern))
                    {
                        ...
                        if (NANDCMD_RESCAN == gClearBBT)
                            printk(KERN_INFO "Found bad block at offset "
                                "%0llx\n", page);
                        break;
                    }
                }
                ...
                {
                    printk(KERN_DEBUG "brcmnand_read_pageoob returns %d for "
                        "page %0llx\n", res, page);
                }
            }
        }
...
/**
 * brcmnand_default_bbt - [NAND Interface] Select a default bad block table
 *  for the device
 * @mtd:    MTD device structure
 *
...
int brcmnand_default_bbt (struct mtd_info *mtd)
{
    ...
    /* THT: 8/18/08: For MLC flashes with block size of 512KB, allocate 8
     * blocks or 4MB, (this is possible because this region is outside of
     * the CFE allocated space of 1MB at 1FF0_0000).
     */
    else
    {
        this->bbt_td->maxblocks = this->bbt_md->maxblocks = 
