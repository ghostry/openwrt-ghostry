...
---------------------------------------------
In case of a single bit correctable error, the block in which correctable
error occured is refreshed (i.e., read->erase->write the entire block).
Following a refresh a success value is returned by brcmnand_read() i.e.,
the error is hidden from the file system. The Correctable Error Table (CET)
keeps a history (bit-vector) of per page correctable errors. If a correctable
error happens on the same page twice, an err is returned to the file system.
...
#define CET_START_BLK_SLC(x, y) \
    (uint32_t) (((x) >> ((y)->bbt_erase_shift)) -  \
    (BBT_SLC_PARTITION/(y)->blockSize))

#define BBT_MLC_PARTITION   (4<<20)
#define BBT_MAX_BLKS_MLC(x) (BBT_MLC_PARTITION >> ((x)->bbt_erase_shift))
#define CET_START_BLK_MLC(x, y, z)  \
    (uint32_t) (((x) >> ((y)->bbt_erase_shift)) - ((z)/(y)->blockSize))
...
static int search_cet_blks(struct mtd_info *, struct brcmnand_cet_descr *,
    char);
 ...
 */
static inline int brcmnand_cet_read_oob(struct mtd_info *mtd, uint8_t *buf,
    loff_t offs)
{
 ...
 */
static inline int brcmnand_cet_write_oob(struct mtd_info *mtd, uint8_t *buf,
    loff_t offs)
{
    struct mtd_oob_ops ops;
 ...
 */
static void cet_printpg_oob(struct mtd_info *mtd,
    struct brcmnand_cet_descr *cet, int count)
{
 ...
 */ 
static void cet_printblk_oob(struct mtd_info *mtd,
    struct brcmnand_cet_descr *cet)
{
    ...
    {
        memset(oobbuf, 0, mtd->oobsize);
        offs = ((loff_t) cet->startblk+((cet->sign)*i)) <<
...
static void cet_eraseall(struct mtd_info *mtd, struct brcmnand_cet_descr *cet)
{
    ...
    for (i = 0; i < cet->numblks; i++)
    {
        if (cet->memtbl[i].blk != -1)
        {
            ...
            if (unlikely(ret < 0))
            {
                printk(KERN_ERR "brcmnandCET: %s Error erasing block %lx\n",
                    __FUNCTION__, (unsigned long) einfo.addr);
            }
        }
    }
    return;
 ...
 */
static inline void cmdline_showcet(struct mtd_info *mtd,
    struct brcmnand_cet_descr *cet)
{
    int i;
    loff_t offs;
    ...
    {
        printk(KERN_INFO "brcmnandCET: Deferred until next correctable "
            "error\n");
        return;
    }
    printk(KERN_INFO "brcmnandCET: Displaying first OOB area of all CET "
        "blocks ...\n");
    for (i = 0; i < cet->numblks; i++)
    {
        if (cet->memtbl[i].blk == -1)
 ...
 */
static inline int cmdline_resetcet(struct mtd_info *mtd,
    struct brcmnand_cet_descr *cet)
{
    int i;
 ...
 */
static int create_cet_blks(struct mtd_info *mtd,
    struct brcmnand_cet_descr *cet)
{
    ...
    for (i = 0; i < td->maxblocks; i++)
    {
        ...
        /* If this is the first CET block, init the correctable erase count
         * to 0
         */
        if (count == 0)
        {
            ...
            *((uint32_t *) (oobptr + cet->offs)) = 0x00000000;
            ...
            if (unlikely(gdebug))
            {
                printk(KERN_INFO "DEBUG -> 0: from = %x\n",
                    (unsigned int) from);
                printk(KERN_INFO "brcmnandCET: Writing cer_count to page "
                    "%x\n", (unsigned int) from);
            }
            if (brcmnand_cet_write_oob(mtd, oobbuf, from))
            {
                printk(KERN_INFO "brcmnandCET: %s Error writing to OOB# "
                    "%x\n", __FUNCTION__, (unsigned int)from);
                return -1;
            }
        }
 ...
 */
static int search_cet_blks(struct mtd_info *mtd,
    struct brcmnand_cet_descr *cet, char force)
{
    int i, count = 0, ret;
    loff_t from;
    struct nand_bbt_descr *td, *md;
    ...
    for (i = 0; i < td->maxblocks; i++)
    {
        ...
        if (count == 0)
        {
            ...
            if (unlikely(gdebug))
            {
                printk(KERN_INFO "brcmnandCET: correctable error count = "
                    "%x\n", cet->cerr_count);
            }
    ...
    for (i = 0; i < cet->numblks; i++)
    {
        if (cet->memtbl[i].isdirty && cet->memtbl[i].blk != -1)
        {
            ...
            for (j = 0; j < mtd->erasesize/this->pageSize; j++)
            {
                ...
                ret = brcmnand_cet_write(mtd, to, (size_t) this->pageSize,
                    cet->memtbl[i].bitvec+pg_idx, oobbuf);
                if (ret < 0)
                {
                    printk(KERN_ERR "brcmnandCET: %s Error writing to page "
...
static void sync_cet(struct work_struct *work)
{
    ...
    struct brcmnand_cet_descr *cet =
        container_of(dwork, struct brcmnand_cet_descr, cet_flush);
    struct mtd_info *mtd = (struct mtd_info *) cet->mtd;

    /* Check if all blocks are clean */
    for (i = 0; i < cet->numblks; i++)
    {
        if (cet->memtbl[i].isdirty)
            break;
    }
...
int brcmnand_create_cet(struct mtd_info *mtd)
{
    ...
    {
        printk(KERN_INFO "brcmnandCET: per chip CET not supported, "
            "disabling\n");
        cet->flags = BRCMNAND_CET_DISABLED;
        return -1;
    }
    ...
    if (NAND_IS_MLC(this))
    {
        ...
        {
            if (cet->maxblks + BBT_MAX_BLKS_MLC(this) >
                BBT_MLC_PARTITION/this->blockSize)
            {
                printk(KERN_INFO "brcmnandCET: Not enough space to store "
                    "CET, disabling CET\n");
                cet->flags = BRCMNAND_CET_DISABLED;
                return -1;
            }
            ...
            if (cet->sign)
            {
                cet->startblk = CET_START_BLK_MLC(this->mtdSize, this,
                    BBT_MLC_PARTITION);
            }
            else
            {
                cet->startblk = (uint32_t) (this->mtdSize >>
    ...
    else
    {
        if (this->mtdSize < (1<<29))
        {
            if (cet->maxblks + BBT_MAX_BLKS_SLC >
                BBT_SLC_PARTITION / this->blockSize)
            {
                printk(KERN_INFO "brcmnandCET: Not enough space to store"
                    " CET, disabling CET\n");
                cet->flags = BRCMNAND_CET_DISABLED;
            ...
            {
                cet->startblk = (uint32_t)
                    (this->mtdSize >> this->bbt_erase_shift)-1;
            }
        }
        else
        {
    ...
    cet->memtbl = (struct brcmnand_cet_memtable *)
        vmalloc(cet->numblks*sizeof(struct brcmnand_cet_memtable));
    if (cet->memtbl == NULL)
    {
    ...
    if (unlikely(gClearCET == 1))
    {

        /* kernel cmdline showcet */
        cmdline_showcet(mtd, cet);
    }
    if (unlikely(gClearCET == 2))
    {

        /* kernel cmdline resetcet */
    ...
    if (unlikely(gClearCET == 3))
    {

        /* kernel cmdline disable */
        cet->flags = BRCMNAND_CET_DISABLED;
        ret = -1;
    }
    //cet_printpg_oob(mtd, cet, 3);
    ...
    default:
        printk(KERN_INFO "brcmnandCET: Status -> Fatal error CET "
            "disabled\n");
        cet->flags = BRCMNAND_CET_DISABLED;
        break;
    }
 ...
 * flag == BRCMNAND_CET_LAZY => correctable error count is 0 so need of
 *      callback
 * 
...
int brcmnand_cet_erasecallback(struct mtd_info *mtd, u_int32_t addr) 
{
    ...
    struct brcmnand_cet_descr *cet = this->cet;
    uint32_t page = 0;
    int blkbegin, blk, i, ret, retlen, pg_idx = 0, numzeros = 0;
    int byte, gdebug = 0;
    uint32_t *ptr;
    unsigned int pos;
    ...
    if (cet->memtbl[blk].bitvec == NULL)
    {
        if (gdebug)
        {
            printk(KERN_INFO "DEBUG -> brcmnandCET: bitvec is null,"
                " reloading\n");
        }
        ...
        for (i = 0; i < mtd->erasesize/mtd->writesize; i++)
        {
            if (gdebug)
            {
                printk(KERN_INFO "DEBUG -> brcmnandCET: Reading page "
                    "%d\n", i);
            }
            ret = mtd->read(mtd, addr, this->pageSize, &retlen,
                (uint8_t *) (cet->memtbl[blk].bitvec+pg_idx));
    ...
    page = (uint32_t)
        ((origaddr & (~(mtd->erasesize-1))) >> this->page_shift);
    ...
    if (likely(numzeros == 0))
    {
        if (gdebug)
        {
            printk(KERN_INFO "DEBUG -> brcmnandCET: returning 0 numzeros"
                " = 0\n");
        }
        return 0;
    }
    if (cet->cerr_count < numzeros)
    {
        if (gdebug)
        {
            printk(KERN_ERR "brcmnandCET: Erroneous correctable error"
                " count");
        }
        return -1;
    }
    ...
    memset(cet->memtbl[blk].bitvec+byte, 0xff,
        (mtd->erasesize/mtd->writesize)>>3);
...
int brcmnand_cet_update(struct mtd_info *mtd, loff_t from, int *status) 
{
    ...
    if (cet->memtbl[blk].bitvec == NULL)
    {
        if (gdebug)
        {
            printk(KERN_INFO "DEBUG -> brcmnandCET: bitvec null "
                ".... loading ...\n");
        }
        cet->memtbl[blk].bitvec = (char *) vmalloc(this->blockSize);
        if (cet->memtbl[blk].bitvec == NULL)
        ...
        if (gdebug)
        {
            printk(KERN_INFO "DEBUG -> brcmnandCET: Reading pages "
                "starting @ %x\n", (unsigned int) from);
        }
        for (i = 0; i < mtd->erasesize/mtd->writesize; i++)
        {
            ret = mtd->read(mtd, from, this->pageSize, &retlen,
                (uint8_t *) (cet->memtbl[blk].bitvec+pg_idx));
            if (ret < 0 || (retlen != this->pageSize))
            {
                vfree(cet->memtbl[blk].bitvec);
    ...
    if ((c & mask) == mask)
    {
        ...
        /* First time error mark it but return a good status */
        *status = 0;
        c = (c & ~mask);
    ...
    {
        /* This page had a previous error so return a bad status */
        *status = 1;
    }
    cet->cerr_count++;
    return 0;
...
/* brcmnand_cet_prepare_reboot Call flush_memcet to flush any in-mem
 *  dirty data
 * 
 * @param mtd       MTD device structure
 * 
...
int brcmnand_cet_prepare_reboot(struct mtd_info *mtd) 
{
    ...
    if (unlikely(gdebug))
    {
        printk(KERN_INFO "DEBUG -> brcmnandCET: flushing pending "
            "CET\n");
    }
